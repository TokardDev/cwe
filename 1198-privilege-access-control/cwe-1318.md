...
|-------|-------------|----------|
| Architecture and Design | Use bus fabrics that natively support security attribute signals (e.g., AXI AxPROT, OCP MReqInfo) for all interconnects carrying transactions to security-sensitive peripherals. | Critical |
| Architecture and Design | When a legacy bus without security signals is unavoidable, insert a security bridge or firewall between the fabric and all security-sensitive slaves to enforce access control. | Critical |
| Implementation | Enable and correctly configure all optional security attribute signals (e.g., PPROT, MReqInfo) during RTL generation; never leave them unconfigured or disabled. | High |
| Architecture and Design | Segregate security-sensitive peripherals onto a separate bus domain with full security signal support, isolating them from non-secure peripherals on simpler buses. | High |
